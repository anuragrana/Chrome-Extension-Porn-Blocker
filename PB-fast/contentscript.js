...
function saveLogNow() {
	var d = new Date();
	var dateString = d.getDate() + "/"+addZero((d.getMonth()+1))+ "/"+d.getFullYear() + "  " +addZero(d.getHours()) + " : " +addZero(d.getMinutes());
	var newLog = url + "|ZZZ|" + dateString;
	chrome.storage.local.get('logs', function (result) {
		logs = result.logs;
		if(logs) {
			logs = logs + "|YYY|" + newLog;
		} else {
			logs = newLog;
		}
